...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('Welcome back to TRA.P!', 'success')
    ...
    >
      ...
      <div className="bg-white border border-border-default rounded-md p-8 md:p-12 relative flex flex-col gap-8 shadow-sm hover:border-primary transition-colors duration-300 group">
        ...
        <div className="text-center space-y-2 mt-2">
          ...
          <p className="font-sans text-xs text-text-secondary uppercase tracking-widest">Access Intelligence</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 text-center">
          <a className="font-mono text-[10px] font-bold text-text-secondary hover:text-primary transition-colors uppercase tracking-wider" href="#">Forgot Password?</a>
          <Link className="font-mono text-[10px] font-bold text-text-secondary hover:text-primary transition-colors uppercase tracking-wider" to="/register">Create Account</Link>
        </div>
      </div>
    </motion.div>
  )
